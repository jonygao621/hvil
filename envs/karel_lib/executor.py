...
from . import KarelForSynthesisParser
from . import KarelSyntaxError
...
def evaluate_code(code, arguments, tests, do_execute):
    stats = {'total': len(tests), 'correct': 0, 'exceptions': 0,
             ...
             'correct_each': []}
    if not code:
        stats['correct_each'] = [False] * len(tests)
        return stats
    for test in tests:
        stats['correct_each'].append(False)
        try:
