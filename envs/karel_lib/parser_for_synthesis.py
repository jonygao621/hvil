...
from .parser_base import parser_prompt
    ...
    def p_if(self, p):
        ...
        if self.build_tree:
            ...
            def if_():
                ...
                self.karel.block_event_callback('if', span, cond_value, 'start')
                if cond_value:
                    stmt()
                self.karel.block_event_callback('if', span, cond_value, 'end')
        p[0] = if_
        ...
        if self.build_tree:
            ...
            def ifelse():
                ...
                self.karel.event_callback('ifElse', span, cond_span, cond_value,
                        true_span if cond_value else false_span)
                self.karel.block_event_callback(
                        'ifElse', span, cond_value, 'start')
                if cond_value:
                    stmt1()
                else:
                    stmt2()
                self.karel.block_event_callback(
                        'ifElse', span, cond_value, 'end')
            ...
            def while_():
                while True:
                    ...
                    self.karel.event_callback('while', span, cond_span, cond_value,
                            true_span if cond_value else false_span)
                    self.karel.block_event_callback(
                            'while', span, cond_value, 'enter')
                    if not cond_value:
                        break
                    stmt()
                self.karel.block_event_callback(
                        'while', span, cond_value, 'exit')
            ...
            def repeat():
                for i in range(limit, 0, -1):
                    self.karel.event_callback('repeat', span, cste.span, i,
                                              true_span)
                    self.karel.block_event_callback('repeat', span,  i, 'enter')
                    stmt()
                self.karel.event_callback('repeat', span, cste.span, 0,
                        false_span)
                self.karel.block_event_callback('repeat', span, i, 'exit')
        p[0] = repeat
